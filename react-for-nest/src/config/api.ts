import { IBackendRes, ICompany, IAccount, IUser, IModelPaginate, IGetAccount, IJob, IResume, IPermission, IRole, ISubscribers, IPost } from '@/types/backend';
...
export const callForgotPassword = (username: string) => {
    return axios.post<IBackendRes<IAccount>>('/api/v1/auth/forgot-password', { email: username })
}

export const callVerifyOtp = (otpCode: string, hash: string) => {
    return axios.post<IBackendRes<IAccount>>('/api/v1/auth/verify-otp', { otpCode, hash })
}

export const callChangePassword = (password: string, confirmPassword: string, hash: string) => {
    return axios.post<IBackendRes<IAccount>>('/api/v1/auth/reset-password', { password, confirmPassword, hash })
}
...
Module Post
 */
export const callCreatePost = (title: string, description: string, content: string, image: string) => {
    return axios.post<IBackendRes<IPost>>('/api/v1/posts', { title, description, content, image })
}

export const callUpdatePost = (id: string, title: string, description: string, content: string, image: string) => {
    return axios.patch<IBackendRes<IPost>>(`/api/v1/posts/${id}`, { title, description, content, image })
}
